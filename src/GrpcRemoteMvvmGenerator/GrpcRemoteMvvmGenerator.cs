    ...
    public class GrpcRemoteMvvmGenerator : IIncrementalGenerator
    {
        ...
        private void Execute(Compilation compilation, System.Collections.Immutable.ImmutableArray<ClassDeclarationSyntax> classes, SourceProductionContext context, AnalyzerConfigOptionsProvider optionsProvider)
        {
            ...
            foreach (var classSyntax in classes)
            {
                ...
                var genServer = grpcServices is not null && (grpcServices.Equals("Server", StringComparison.OrdinalIgnoreCase) || grpcServices.Equals("Both", StringComparison.OrdinalIgnoreCase));
                ...
                string viewModelCode = GenerateViewModelPartialClass(originalViewModelName, classSymbol.ContainingNamespace.ToDisplayString(), serverImplNamespace, clientProxyNamespace, genServer);
        ...
        private string GenerateViewModelPartialClass(string viewModelName, string viewModelNamespace, string serverImplNamespace, string clientProxyNamespace, bool genServer)
        {
            var sb = new StringBuilder();
