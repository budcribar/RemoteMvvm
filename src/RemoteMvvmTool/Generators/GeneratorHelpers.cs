...
public static class GeneratorHelpers
{
    ...
    public static string ToProtoValue(ITypeSymbol typeSymbol, string expr)
    {
        ...
        if (typeSymbol is INamedTypeSymbol named &&
            (named.TypeKind == TypeKind.Class || named.TypeKind == TypeKind.Struct || named.TypeKind == TypeKind.Error) &&
            !named.ContainingNamespace.ToDisplayString().StartsWith("System", StringComparison.Ordinal))
        {
            var props = Helpers.GetAllMembers(named)
                .OfType<IPropertySymbol>()
                .Where(p => p.GetMethod != null && p.Parameters.Length == 0)
                .ToList();

            var sb = new StringBuilder();
            sb.Append($"new {named.Name}State {{ ");
            for (int i = 0; i < props.Count; i++)
            {
                var p = props[i];
                var inner = ToProtoValue(p.Type, $"{expr}.{p.Name}");
                sb.Append($"{GeneratorHelpers.ToPascalCase(p.Name)} = {inner}");
    ...
    public static string FromProtoValue(ITypeSymbol typeSymbol, string expr)
    {
        if (typeSymbol.TypeKind == TypeKind.Enum)
        {
            return $"({typeSymbol.ToDisplayString()}){expr}";
        }
            ...
            !named.ContainingNamespace.ToDisplayString().StartsWith("System", StringComparison.Ordinal))
        {
            ...
            var sb = new StringBuilder();
            sb.Append($"new {typeSymbol.ToDisplayString()} {{ ");
            for (int i = 0; i < props.Count; i++)
            {
                var p = props[i];
                var inner = FromProtoValue(p.Type, $"{expr}.{GeneratorHelpers.ToPascalCase(p.Name)}");
                sb.Append($"{p.Name} = {inner}");
                if (i < props.Count - 1) sb.Append(", ");
            }
            sb.Append(" }");
            return sb.ToString();
