...
public static class SplitProjectGenerator
{
    ...
    public static GeneratedSplitPaths Generate(string baseDir, string viewModelName, List<PropertyInfo> props, List<CommandInfo> cmds, Compilation compilation, string platform, string protoNamespace = "Test.Protos", bool useServerGui = true)
    {
        ...
        // Choose between GUI and console server program generation
        if (useServerGui)
        {
            if (!string.Equals(platform, "wpf", StringComparison.OrdinalIgnoreCase))
            {
                // For non-WPF (WinForms), generate Program.cs with GUI
                File.WriteAllText(Path.Combine(serverDir, "Program.cs"), CsProjectGenerator.GenerateServerGuiProgram("ServerApp", platform, protoNamespace, serviceName, props, cmds));
            }
            // For WPF, we'll generate XAML files below instead of Program.cs
        }
        else
        {
            // Generate console-only server
        ...
        // Generate WPF XAML files for server if using GUI and WPF platform
        if (useServerGui && string.Equals(platform, "wpf", StringComparison.OrdinalIgnoreCase))
        {
            try
            {
                var appXamlPath = Path.Combine(serverDir, "App.xaml");
                var appCsPath = Path.Combine(serverDir, "App.xaml.cs");
                var mainXamlPath = Path.Combine(serverDir, "MainWindow.xaml");
                var mainCsPath = Path.Combine(serverDir, "MainWindow.xaml.cs");
                File.WriteAllText(appXamlPath, CsProjectGenerator.GenerateServerWpfAppXaml());
                File.WriteAllText(appCsPath, CsProjectGenerator.GenerateServerWpfAppCodeBehind(viewModelName));
                File.WriteAllText(mainXamlPath, CsProjectGenerator.GenerateServerWpfMainWindowXaml("ServerApp", viewModelName, props, cmds));
                File.WriteAllText(mainCsPath, CsProjectGenerator.GenerateServerWpfMainWindowCodeBehind());
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[SplitProjectGenerator] Failed generating server WPF XAML files: {ex.Message}");
            }
        }
        ...
        if (!string.Equals(platform, "wpf", StringComparison.OrdinalIgnoreCase))
        {
            ...
            // For WinForms, use the new abstraction to generate the complete client program
            if (platform.Equals("winforms", StringComparison.OrdinalIgnoreCase))
            {
                var winFormsGenerator = new WinFormsClientUIGenerator("GuiClientApp", viewModelName, props, cmds, viewModelName + "RemoteClient", "Generated.Clients");
                var clientProgram = winFormsGenerator.GenerateProgram(protoNamespace, serviceName);
                File.WriteAllText(Path.Combine(clientDir, "Program.cs"), clientProgram);
            }
        }
        ...
        // Generate required WPF XAML files when platform is WPF (tests/build expect them)
        if (string.Equals(platform, "wpf", StringComparison.OrdinalIgnoreCase))
        {
            try
            {
...
""";
            ...
            // individual project solution files for convenience
