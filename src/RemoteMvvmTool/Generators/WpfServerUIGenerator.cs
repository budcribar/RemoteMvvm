...
public class WpfServerUIGenerator : UIGeneratorBase
{
    ...
    public string GenerateEnhancedServerCodeBehind()
    {
        ...
        sb.AppendLine("            AttachPropertyChangedHandlers(_viewModel);");
        sb.AppendLine();
        ...
        sb.AppendLine("        }");
        sb.AppendLine();
        sb.AppendLine("        private void AttachPropertyChangedHandlers(object? obj)");
        sb.AppendLine("        {");
        sb.AppendLine("            if (obj == null) return;");
        sb.AppendLine();
        sb.AppendLine("            if (obj is INotifyPropertyChanged inpc)");
        sb.AppendLine("            {");
        sb.AppendLine("                inpc.PropertyChanged += (_, __) =>");
        sb.AppendLine("                {");
        sb.AppendLine("                    Dispatcher.BeginInvoke(() =>");
        sb.AppendLine("                    {");
        sb.AppendLine("                        LoadTree();");
        ...
        sb.AppendLine("            }");
        sb.AppendLine();
        sb.AppendLine("            if (obj is INotifyCollectionChanged ncc && obj is System.Collections.IEnumerable enumerable1)");
        sb.AppendLine("            {");
        sb.AppendLine("                ncc.CollectionChanged += (_, e) =>");
        sb.AppendLine("                {");
        sb.AppendLine("                    if (e.NewItems != null)");
        sb.AppendLine("                        foreach (var item in e.NewItems)");
        sb.AppendLine("                            AttachPropertyChangedHandlers(item);");
        sb.AppendLine("                    Dispatcher.BeginInvoke(() =>");
        sb.AppendLine("                    {");
        sb.AppendLine("                        LoadTree();");
        sb.AppendLine("                        UpdateServerStatus();");
        sb.AppendLine("                    });");
        sb.AppendLine("                };");
        sb.AppendLine("                foreach (var item in enumerable1)");
        sb.AppendLine("                    AttachPropertyChangedHandlers(item);");
        sb.AppendLine("                return;");
        sb.AppendLine("            }");
        sb.AppendLine();
        sb.AppendLine("            if (obj is System.Collections.IEnumerable enumerable && obj is not string)");
        sb.AppendLine("            {");
        ...
        sb.AppendLine("            }");
        sb.AppendLine();
        sb.AppendLine("            foreach (var p in obj.GetType().GetProperties())");
        sb.AppendLine("            {");
        sb.AppendLine("                if (p.GetIndexParameters().Length > 0) continue;");
        sb.AppendLine("                var val = p.GetValue(obj);");
        sb.AppendLine("                AttachPropertyChangedHandlers(val);");
        sb.AppendLine("            }");
        sb.AppendLine("        }");
        sb.AppendLine("    }");
        sb.AppendLine("}");
