...
public class GrpcWinformsEndToEndTests
{
    ...
    private static void FailIfNamedGuiProcessesRunning()
    {
        ...
        foreach (var name in names)
        {
            try
            {
                ...
                {
                    Console.WriteLine($"[GrpcWinformsEndToEndTests] Found {processes.Length} leftover {name} process(es). Attempting cleanup...");
                    ...
                    foreach (var process in processes)
                    {
                        try
                        {
                            ...
                            {
                                Console.WriteLine($"[GrpcWinformsEndToEndTests] Killing leftover process: {name} (PID: {process.Id})");
                        ...
                        {
                            Console.WriteLine($"[GrpcWinformsEndToEndTests] Failed to kill {name}: {ex.Message}");
                        }
                        finally
                        {
                    ...
                    Console.WriteLine($"[GrpcWinformsEndToEndTests] Successfully cleaned up leftover {name} processes.");
                }
            }
    ...
    // Restore: Edge case primitives structural verification
    [Fact]
    public async Task EdgeCasePrimitives_Winforms_EndToEnd_Test()
    {
        ...
        await ModelVerifier.VerifyModelStructuralAsync(
            ...
            "Split WinForms edge primitives");
        ctx.MarkTestPassed();
    }

    // Restore: Nested custom objects structural verification
    [Fact]
