        ...
        if (typeof value === 'string') {
            ...
            if (Number.isInteger(value)) {
                if (value > 2147483647 || value < -2147483648) {
                    const wrapper = new google_protobuf_google_protobuf_wrappers_pb__WEBPACK_IMPORTED_MODULE_3__.Int64Value();
                    wrapper.setValue(value);
                ...
                const wrapper = new google_protobuf_google_protobuf_wrappers_pb__WEBPACK_IMPORTED_MODULE_3__.DoubleValue();
                wrapper.setValue(value);
            ...
            const wrapper = google_protobuf_google_protobuf_timestamp_pb__WEBPACK_IMPORTED_MODULE_4__.Timestamp.fromDate(value);
...
class HP3LSThermalTestViewModelServiceClient {
    ...
    getState(request, metadata, callback) {
        if (callback !== undefined) {
            const wrappedCallback = (err, response) => {
                if (err) {
                    // Log RPC errors for easier diagnostics
    ...
    subscribeToPropertyChanges(request, metadata) {
        ...
        // Attach error logging on the stream to surface server streaming failures
