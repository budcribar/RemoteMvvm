...
{
    public partial class ThermalZoneComponentViewModel : CommunityToolkit.Mvvm.ComponentModel.ObservableObject, IDisposable
    {
        private ThermalZoneComponentViewModelGrpcServiceImpl? _grpcService;
        private IHost? _aspNetCoreHost;
        private GrpcChannel? _channel;
        private HPSystemsTools.ViewModels.RemoteClients.ThermalZoneComponentViewModelRemoteClient? _remoteClient;
        private readonly Dispatcher _dispatcher;

        public ThermalZoneComponentViewModel(ServerOptions options) : this()
        {
            if (options == null) throw new ArgumentNullException(nameof(options));
            _dispatcher = Dispatcher.CurrentDispatcher;
            _grpcService = new ThermalZoneComponentViewModelGrpcServiceImpl(this, _dispatcher);
        ...
        private void StartAspNetCoreServer(ServerOptions options)
        {
            ...
            // Map gRPC services
            app.MapGrpcService<ThermalZoneComponentViewModelGrpcServiceImpl>()
               .EnableGrpcWeb()
        ...
        public ThermalZoneComponentViewModel(ClientOptions options) : this()
        {
            if (options == null) throw new ArgumentNullException(nameof(options));
            _dispatcher = Dispatcher.CurrentDispatcher;
            _channel = GrpcChannel.ForAddress(options.Address);
            var client = new ThermalZoneComponentViewModelService.ThermalZoneComponentViewModelServiceClient(_channel);
            _remoteClient = new ThermalZoneComponentViewModelRemoteClient(client);
        }

        public async Task<ThermalZoneComponentViewModelRemoteClient> GetRemoteModel()
        {
