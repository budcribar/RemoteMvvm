...
public partial class SupportedComplexViewModelGrpcServiceImpl : SupportedComplexViewModelService.SupportedComplexViewModelServiceBase
{
    ...
    static SupportedComplexViewModelGrpcServiceImpl()
    {
        ClientCount = 0;
    }

    private readonly SupportedComplexViewModel _viewModel;
    private static readonly ConcurrentDictionary<IServerStreamWriter<ComplexTypes.Protos.PropertyChangeNotification>, Channel<ComplexTypes.Protos.PropertyChangeNotification>> _subscriberChannels = new ConcurrentDictionary<IServerStreamWriter<ComplexTypes.Protos.PropertyChangeNotification>, Channel<ComplexTypes.Protos.PropertyChangeNotification>>();
    private readonly Dispatcher _dispatcher;
    private readonly ILogger? _logger;

    public SupportedComplexViewModelGrpcServiceImpl(SupportedComplexViewModel viewModel, Dispatcher dispatcher, ILogger<SupportedComplexViewModelGrpcServiceImpl>? logger = null)
    {
    ...
    public override Task<SupportedComplexViewModelState> GetState(Empty request, ServerCallContext context)
    {
        var state = new SupportedComplexViewModelState();
        // Mapping property: Layers to state.Layers
        try
        {
    ...
    {
        var channel = Channel.CreateUnbounded<ComplexTypes.Protos.PropertyChangeNotification>(new UnboundedChannelOptions { SingleReader = true, SingleWriter = false });
    ...
    private async void ViewModel_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        catch (Exception ex) { Debug.WriteLine("[GrpcService:SupportedComplexViewModel] Error getting property value for " + e.PropertyName + ": " + ex.Message); return; }

        var notification = new ComplexTypes.Protos.PropertyChangeNotification { PropertyName = e.PropertyName };
        ...
        foreach (var channelWriter in _subscriberChannels.Values.Select(c => c.Writer))
        {
            ...
            catch (ChannelClosedException) { Debug.WriteLine("[GrpcService:SupportedComplexViewModel] Channel closed for a subscriber, cannot write notification for '" + e.PropertyName + "'. Subscriber likely disconnected."); }
            catch (Exception ex) { Debug.WriteLine("[GrpcService:SupportedComplexViewModel] Error writing to subscriber channel for '" + e.PropertyName + "': " + ex.Message); }
        }
    }
}
